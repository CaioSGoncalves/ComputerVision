...
class Window:
    ...
    def __init__(self, img, x, y, half_length):    
        self.x_center = x
        self.y_center = y
    ...
    def def_cordinates_and_values(self):
        x1 = self.x_center - self.half_length
        x2 = self.x_center + self.half_length
        y1 = self.y_center - self.half_length
        y2 = self.y_center + self.half_length
        self.top_left = (x1,y1)
    ...
    def put_cordinates(self, color):
        ...
        cordinate = (self.x_center-(120),self.y_center-(self.half_length+4))
    ...
    def put_rgb_values(self, color):
        ...
        cordinate = (self.x_center-(120),self.y_center+(self.half_length+16))
        cv2.putText(self.image,text,cordinate,font,0.5,color,1,cv2.LINE_AA)
    ...
    def put_mean_std(self, color):
        ...
        cordinate = (self.x_center-(120),self.y_center+(self.half_length+32))
        cv2.putText(self.image,text,cordinate,font,0.5,color,1,cv2.LINE_AA)
    ...
    def it_fits(self):
        ...
        x_len_fits = window_shape[0] == 2*self.half_length + 1
        y_len_fits = window_shape[1] == 2*self.half_length + 1
